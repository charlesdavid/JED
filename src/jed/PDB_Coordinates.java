...
/**
 * JED class PDB_Coordinates: Data structure for the Alpha Carbon Coordinates in a PDB file. Note: PCA is only done for the Alpha Carbons in this version.
 * Copyright (C) 2012 Dr.
 * Charles David
 * 
...
class PDB_Coordinates
	{
		...
		public static Matrix get_PDB_Coords(Vector<Atom> atoms) // must pass in a vector of CA atoms only
			{
				int i = 0;
				int num = atoms.size();
				Matrix col = new Matrix(3 * num, 1);
				for (Atom a : atoms)
					{
						double x = a.x;
						double y = a.y;
