...
/**
 * JED class FES_Driver: Driver program for getting the free energy of two order parameters using 2D KDE This class imports bits/kde developed by Philip DeCamp.
 * 
		...
		private static void write_Logs()
			{
				...
				Job_log_writer.write("The bounds for the KDE are: OP1(" + nf.format(KDE_bounds[0]) + "," + nf.format(KDE_bounds[2]) + "); OP2("
						+ nf.format(KDE_bounds[1]) + "," + nf.format(KDE_bounds[3]) + ")" + "\n");
				...
				Batch_log_Writer
						.write(String.format("%-1s%-16s%-8s%-8s%-8s%-8s%-8s%-12s%-12s%-12s%-8s%-4s", description, "Conformations =",
								order_param1.getRowDimension(), "OP1 =", OP1, "OP2 =", OP2, "Bounds are: OP1(",
								nf.format(KDE_bounds[0]) + "," + nf.format(KDE_bounds[2]) + "); OP2(" + nf.format(KDE_bounds[1]) + ","
										+ nf.format(KDE_bounds[3]),
								") ", "KDE Bandwidths are: ", nf.format(KDE_bandwidths[0])) + ", " + nf.format(KDE_bandwidths[3]) + "\n");
				date = DateUtils.now();
		...
		private void get_FES()
			{
				...
				System.out.println(
						"The bounds for the KDE are: OP1(" + KDE_bounds[0] + "," + KDE_bounds[2] + "); OP2(" + KDE_bounds[1] + "," + KDE_bounds[3] + ")");
				...
				for (job_number = 0; job_number < (num_of_jobs); job_number++)
					{
						read_job_parameters();
						initialize_Job_Log();
						FES_Driver fes = new FES_Driver(delta_vectors, OP1, OP2, number_of_points, OP_offset, cellsize, directory, out_directory, description);
						fes.get_FES();
						write_Logs();
