...
/**
 * JED class JED_Batch_Driver: Driver program for running JED in (sequential) batch mode. Input file is "JED_Batch_Driver.txt" Copyright (C) 2012 Dr. Charles David
 *
...
public class JED_Batch_Driver
{
	...
	static int number_Of_Input_Lines, line_count, num_of_jobs, job_num, number_of_modes_cartesian, number_of_modes_dist_pairs, number_of_residue_pairs, number_of_modes_viz,
			number_of_residues_cartesian, ROWS, COLS;
	static String input_path, path, name, directory, out_dir, rl_cartesian, rl_dist_pairs, coord_input_file, description, pdb_ref_file, line, date, Q = "COV", R = "CORR",
			P = "PCORR";
	static double trace_COV, trace_d_cov, trace_CORR, trace_d_corr, trace_PCORR, trace_d_pcorr, cond_cov, det_cov, rank_cov, cond_d_cov, det_d_cov, rank_d_cov, mode_amplitude,
			z_cut, percent;
	...
	static double[] pca_mode_min_COV, pca_mode_max_COV, pca_mode_min_CORR, pca_mode_max_CORR, pca_mode_max_PCORR, pca_mode_min_PCORR, residue_distance_means,
			residue_distance_std_devs;
	static Matrix original_reference_PDB_coordinates, original_PDB_coordinates, subset_PDB_coordinates_cartesian, weighted_pca_modes_COV, square_pca_modes_COV,
			weighted_square_pca_modes_COV, transformed_PDB_coordinates_cartesian, distance_matrix, top_cartesian_evectors_COV, top_distance_evectors_COV, projections_COV,
			normed_projections_COV, weighted_pca_modes_CORR, square_pca_modes_CORR, weighted_square_pca_modes_CORR, top_cartesian_evectors_CORR, top_distance_evectors_CORR,
			projections_CORR, normed_projections_CORR, weighted_normed_projections_COV, weighted_normed_projections_CORR, pca_modes_CORR, pca_modes_COV,
			top_cartesian_evectors_PCORR, top_distance_evectors_PCORR, square_pca_modes_PCORR;
	...
	private static void read_analytical_job_parameters()
		{
			...
			if (!read_PDBs)
			{
				...
				if (count < 1)
				{
					System.err.println("Expected 1 parameter: Coordinates Matrix, but only got: " + count);
					System.err.println("Terminating program execution.");
					System.exit(0);
				}
	...
	private static void write_REF_Read_Log()
		{
			try
			{
				...
				if (do_multi)
					log_writer.write("The file " + name + "' contains all chainID-residue number pairs found in the PDB file, formatted for Multi Chain Analysis Input." + "\n");
				log_writer.flush();
	...
	private static void do_cPCA()
		{
			...
			JED_Do_Cartesian cSS = new JED_Do_Cartesian(directory, description, pdb_ref_file, rl_cartesian, number_of_modes_cartesian, original_PDB_coordinates,
	...
	private static void get_cPCA_SSA()
		{
			...
			JED_Get_Subspace_Analysis jssa2 = new JED_Get_Subspace_Analysis(directory, description, "cPCA", top_cartesian_evectors_COV, top_cartesian_evectors_PCORR,
					"COV_vs_PCORR");
			{
				jssa2.get_SSA_JED();
			...
			JED_Get_Subspace_Analysis jssa3 = new JED_Get_Subspace_Analysis(directory, description, "cPCA", top_cartesian_evectors_CORR, top_cartesian_evectors_PCORR,
					"CORR_vs_PCORR");
			{
				jssa3.get_SSA_JED();
	...
	private static void write_cPCA_Log()
		{
			try
			{
				...
				if (percent > 0)
					log_writer.write("The transformed data was trimmed by removing " + (percent * 100) + " percent of the samples based on conformation RMSD." + "\n");
				else
					log_writer.write("No samples were removed from the data" + "\n");
				if (z_cut > 0)
					log_writer.write("The coordinate values with Z-scores beyond |" + z_cut + "| were set to their mean value.\n\n");
				else
				...
				log_writer.write(
						"Comparators include RMSIP and Principle Angles, for the essential subspace and iterated comparisons from dim 1 to " + number_of_modes_cartesian + "\n");
	...
	private static void do_dpPCA()
		{
			JED_Do_Dist_Pairs dpSS = new JED_Do_Dist_Pairs(directory, description, pdb_ref_file, rl_dist_pairs, number_of_modes_dist_pairs, original_PDB_coordinates,
					original_reference_PDB_coordinates);
	...
	private static void get_dpPCA_SSA()
		{
			...
			JED_Get_Subspace_Analysis jssa2 = new JED_Get_Subspace_Analysis(directory, description, "dpPCA", top_distance_evectors_COV, top_distance_evectors_PCORR,
					"COV_vs_PCORR");
			{
				jssa2.get_SSA_JED();
				jssa2.get_FSSA_Iterated_JED();
			...
			JED_Get_Subspace_Analysis jssa3 = new JED_Get_Subspace_Analysis(directory, description, "dpPCA", top_distance_evectors_CORR, top_distance_evectors_PCORR,
					"CORR_vs_PCORR");
			{
				jssa3.get_SSA_JED();
				jssa3.get_FSSA_Iterated_JED();
	...
	private static void write_dpPCA_Log_Multi()
		{
			try
			{
				...
				if (z_cut > 0)
					log_writer.write("The distance values with Z-scores beyond |" + z_cut + "| were set to their mean value.\n\n");
				else
					log_writer.write("No coordinate outliers were adjusted.\n");
				...
				for (int i = 0; i < number_of_residue_pairs; i++)
				{
					log_writer.write(String.format("%-12s%-16s%-16s%-16s%n", chain_idents1.get(i) + residue_list_dp_original1.get(i),
							chain_idents2.get(i) + residue_list_dp_original2.get(i), nf.format(residue_distance_means[i]), nf.format(residue_distance_std_devs[i])));
				}
				...
				log_writer.write(
						"Comparators include RMSIP and Principle Angles, for the essential subspace and iterated comparisons from dim 1 to " + number_of_modes_dist_pairs + "\n");
				log_writer.write("Additional log files can be found in the /SSA directory tree." + "\n\n");
				log_writer.flush();
	...
	private static void do_NO_PCA()
		{
			...
			if (do_multi)
			{
				JED_Do_No_PCA npca = new JED_Do_No_PCA(directory, rl_cartesian, chain_ids_read, residues_read, description, pdb_ref_file, original_PDB_coordinates,
						original_reference_PDB_coordinates);
				{
					npca.set_percent_cutoff(percent);
					npca.set_z_cutoff(z_cut);
	...
	private static void do_Mode_Visualization()
		{
			JED_PCA_Mode_Vizualization cov = new JED_PCA_Mode_Vizualization(directory, description, atoms, top_cartesian_eigenvalues_COV, top_cartesian_evectors_COV,
					square_pca_modes_COV, pca_mode_max_COV, pca_mode_min_COV, mode_amplitude, number_of_modes_viz, Q);
			cov.get_Mode_Visualizations_SS();
			cov.get_Essential_Visualization();

			System.gc();

			JED_PCA_Mode_Vizualization corr = new JED_PCA_Mode_Vizualization(directory, description, atoms, top_cartesian_eigenvalues_CORR, top_cartesian_evectors_CORR,
			...
			JED_PCA_Mode_Vizualization pcorr = new JED_PCA_Mode_Vizualization(directory, description, atoms, top_cartesian_eigenvalues_PCORR, top_cartesian_evectors_PCORR,
					square_pca_modes_PCORR, pca_mode_max_PCORR, pca_mode_min_PCORR, mode_amplitude, number_of_modes_viz, P);
	...
	private static void write_VIZ_Log()
		{
			try
			{
				...
				if (number_of_modes_viz >= 5) log_writer
						.write("A set of 20 structures was generated to animate the essential subspace composed of the top 5 modes for the COV, CORR, and PCORR PCA models.\n");
				if (number_of_modes_viz < 5) log_writer.write("A set of 20 structures was generated to animate the essential subspace composed of the top " + number_of_modes_viz
				...
				log_writer.write("For the essential motion, a superposition was created spanning 2 full periods for the first mode.\n");
