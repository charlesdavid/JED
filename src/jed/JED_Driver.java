...
public class JED_Driver
{
	...
	static int number_Of_Input_Lines, line_count, number_of_modes_cartesian, number_of_modes_dist_pairs, number_of_modes_viz, number_of_residues_cartesian, number_of_residue_pairs,
			number_conformations, ROWS, COLS;
	static String input_path, path, name, directory, out_dir, rl_cartesian, rl_dist_pairs, coord_input_file, description, pdb_ref_file, line, date, type, Q = "COV", R = "CORR", P = "PCORR";
	static double trace_COV, trace_d_cov, trace_CORR, trace_d_corr, trace_d_pcorr, trace_PCORR, cond_cov, cond_d_cov, cond_corr, cond_d_corr, cond_pcorr, cond_d_pcorr, det_cov, det_d_cov, det_corr,
			det_d_corr, det_pcorr, det_d_pcorr, rank_cov, rank_d_cov, rank_corr, rank_d_corr, rank_pcorr, rank_d_pcorr, mode_amplitude, z_cut, percent;
	...
	static Matrix original_reference_PDB_coordinates, tranasformed_reference_PDB_coordinates, original_PDB_coordinates, subset_PDB_coordinates_cartesian, weighted_pca_modes_COV, square_pca_modes_COV,
			weighted_square_pca_modes_COV, transformed_PDB_coordinates_cartesian, distance_matrix, top_cartesian_evectors_COV, top_distance_evectors_COV, weighted_pca_modes_CORR,
			square_pca_modes_CORR, square_pca_modes_PCORR, weighted_square_pca_modes_CORR, top_cartesian_evectors_CORR, top_cartesian_evectors_PCORR, top_distance_evectors_CORR,
			top_distance_evectors_PCORR, pca_modes_CORR, pca_modes_COV;
	static Matrix projections_COV, normed_projections_COV, projections_CORR, normed_projections_CORR, weighted_normed_projections_COV, weighted_normed_projections_CORR, weighted_projections_CORR,
			weighted_projections_COV, projections_PCORR, normed_projections_PCORR;
	static Matrix projections_dist_COV, normed_projections_dist_COV, projections_dist_CORR, normed_projections_dist_CORR, weighted_normed_projections_dist_COV, weighted_normed_projections_dist_CORR,
			weighted_projections_dist_CORR, weighted_projections_dist_COV, projections_dist_PCORR, normed_projections_dist_PCORR;
	static Vector<Atom> atoms;
	static boolean read_PDBs, do_cartesian, do_dist_pairs, do_mode_viz, do_multi, do_no_pca, do_fes;
	static File log;
	...
	private static void read_job_parameters()
		{
			...
			out_dir = directory + "JED_RESULTS_" + description + File.separatorChar;
					...
					if (!sToken.hasMoreElements())
						{
							...
							System.err.println("Setting mode apmlitude to default value of 2.0");
							mode_amplitude = 2.0;
						}
					if (sToken.hasMoreElements())
						{
			...
			try
				{
					...
					log_writer.write("JED: Java Essential Dynamics version 1.1" + "\n");
	...
	private static void do_cPCA()
		{
			...
			JED_Do_Cartesian cSS = new JED_Do_Cartesian(directory, description, pdb_ref_file, rl_cartesian, number_of_modes_cartesian, original_PDB_coordinates, original_reference_PDB_coordinates);
	...
	private static void do_NO_PCA()
		{
			if (!do_multi)
				{
					JED_Do_No_PCA npca = new JED_Do_No_PCA(directory, rl_cartesian, residues_read, description, pdb_ref_file, original_PDB_coordinates, original_reference_PDB_coordinates);
						{
	...
	private static void do_Mode_Visualization()
		{
			...
			JED_PCA_Mode_Vizualization pcorr = new JED_PCA_Mode_Vizualization(directory, description, atoms, top_cartesian_eigenvalues_PCORR, top_cartesian_evectors_PCORR, square_pca_modes_PCORR,
					pca_mode_max_PCORR, pca_mode_min_PCORR, mode_amplitude, number_of_modes_viz, P);
			...
			try
				{
					...
					if (number_of_modes_viz >= 5)
						log_writer.write("A set of 20 structures was generated to animate the essential subspace composed of the top 5 modes for the COV, CORR, and PCORR PCA models.\n");
					if (number_of_modes_viz < 5) log_writer.write(
							"A set of 20 structures was generated to animate the essential subspace composed of the top " + number_of_modes_viz + " modes, for the COV, CORR, and PCORR PCA models.\n");
	...
	private static void do_FES()
		{
			if (do_cartesian)
				{
					JED_Get_FES fes = new JED_Get_FES(weighted_projections_CORR, 0, 1, number_conformations, 0, 0, directory, description, "cPCA", R);
					fes.get_FES();
					fes.write_FES_Log();

					fes = new JED_Get_FES(weighted_projections_COV, 0, 1, number_conformations, 0, 0, directory, description, "cPCA", Q);
					fes.get_FES();
					fes.write_FES_Log();

					fes = new JED_Get_FES(projections_PCORR, 0, 1, number_conformations, 0, 0, directory, description, "cPCA", P);
					fes.get_FES();
					fes.write_FES_Log();
				}
			System.gc();
			if (do_dist_pairs)
				{
					JED_Get_FES fes = new JED_Get_FES(weighted_projections_dist_CORR, 0, 1, number_conformations, 0, 0, directory, description, "dpPCA", R);
					fes.get_FES();
					fes.write_FES_Log();

					fes = new JED_Get_FES(weighted_projections_dist_COV, 0, 1, number_conformations, 0, 0, directory, description, "dpPCA", Q);
					fes.get_FES();
					fes.write_FES_Log();

					fes = new JED_Get_FES(projections_dist_PCORR, 0, 1, number_conformations, 0, 0, directory, description, "dpPCA", P);
					fes.get_FES();
					fes.write_FES_Log();
				}
			System.gc();
			...
			try
				{
					...
					log_writer.write("See the FES Logs in the FES subdirectories of each PCA model for more details.\n\n");
					log_writer.flush();
	...
	public static void main(String[] args)
		{
			...
			if (do_fes)
				{
					System.out.print("Performing Free Energy Calculations... ");
					startTime = System.nanoTime();
					do_FES();
					write_FES_Log();
					endTime = System.nanoTime();
