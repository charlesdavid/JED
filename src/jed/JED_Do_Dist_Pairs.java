...
/**
 * JED class JED_Do_Dist_Pairs: Top class for the Distance Pair analysis. Copyright (C) 2012 Dr. Charles David
 *
...
public class JED_Do_Dist_Pairs
	{
		...
		int number_of_Chains, number_of_pairs, number_of_modes_dist_pairs;
		String directory, out_dir, description, pdb_ref_file, rl_dist_pairs, path, line;
		double trace_dist_COV, trace_dist_CORR, trace_dist_PCORR, cond_cov, cond_corr, cond_pcorr, det_cov, det_corr, det_pcorr, rank_cov, rank_corr,
				rank_pcorr, z_cutoff;
		...
		double[] residue_distance_means, residue_distance_std_devs, dist_pca_mode_COV_min, dist_pca_mode_COV_max, dist_pca_mode_CORR_min,
				dist_pca_mode_CORR_max;
		Matrix reference_pdb_coordinates, reference_distances, distance_matrix, distance_matrix_cond, original_PDB_coordinates, subset_PDB_coordinates_dist,
				transformed_subset_PDB_coordinates_dist, cov_dist, corr_dist, top_distance_evectors_COV, top_distance_evectors_CORR,
				top_distance_evectors_PCORR, normed_projections_dist_COV, projections_dist_COV, projections_dist_CORR, normed_projections_dist_CORR,
		...
		public JED_Do_Dist_Pairs(String dir, String desc, String ref_file, String res_dist_pairs, int modes_dist_pairs, Matrix PDB_coordinates,
				Matrix ref_coords)
			{
				super();
				directory = dir;
				...
				reference_pdb_coordinates = ref_coords;
		...
		public void do_Dist()
			{
				...
				get_Reference_Residue_Pair_Distances();
		...
		private void get_Distance_Pair_DVPs()
			{
				JED_Get_Distance_Pair_DVPs SS_dvp_COV = new JED_Get_Distance_Pair_DVPs(distance_matrix, reference_distances, top_distance_evectors_COV,
						top_distance_eigenvalues_COV, directory, description, "COV", number_of_modes_dist_pairs);
					{
						// COVARIANCE METHOD
						SS_dvp_COV.get_Distance_Pair_DV_Series();
					}
				JED_Get_Distance_Pair_DVPs SS_dvp_CORR = new JED_Get_Distance_Pair_DVPs(distance_matrix, reference_distances, top_distance_evectors_CORR,
				...
				JED_Get_Distance_Pair_DVPs SS_dvp_PCORR = new JED_Get_Distance_Pair_DVPs(distance_matrix, reference_distances, top_distance_evectors_CORR,
		...
		private void get_Distance_Pair_PCA()
			{
				JED_Get_Distance_Pair_PCA dp_pca = new JED_Get_Distance_Pair_PCA(distance_matrix_cond, directory, description, number_of_modes_dist_pairs,
						number_of_pairs);
					{
				...
				try
					{
						...
						for (int i = 0; i < number_of_pairs; i++)
							{
									{
										d_stats_writer.write(String.format("%-12s%-16s%-16s%-16s%n", res_list_dist_orig1[i], res_list_dist_orig2[i],
		...
		private void get_Reference_Residue_Pair_Distances()
			{
				JED_Get_Distances_for_Residue_Pairs ref_dist_pairs = new JED_Get_Distances_for_Residue_Pairs(reference_pdb_coordinates, res_list_dist1,
						res_list_dist2, directory, description);
					{
		...
		private void get_Residue_Pair_Distances()
			{
				JED_Get_Distances_for_Residue_Pairs dist_pairs = new JED_Get_Distances_for_Residue_Pairs(original_PDB_coordinates, res_list_dist1,
						res_list_dist2, directory, description);
					{
				...
				try
					{
						...
						for (int i = 0; i < number_of_pairs; i++)
							{
									{
										d_stats_writer.write(String.format("%-12s%-16s%-16s%-16s%n", chain_idents1.get(i) + res_list_dist_orig1[i],
												chain_idents2.get(i) + res_list_dist_orig2[i], nf.format(residue_distance_means[i]),
		...
		private void read_Residues_Pairs_Single()
			{
				JED_Read_Residue_Pair_List rpl = new JED_Read_Residue_Pair_List(directory, description, rl_dist_pairs, pdb_ref_file, original_PDB_coordinates,
