...
public class JED_Batch_Driver
{
	...
	static int number_Of_Input_Lines, line_count, num_of_jobs, job_num, number_of_modes_cartesian, number_of_modes_dist, number_of_modes_dist_pairs, number_of_residue_pairs, number_of_modes_viz,
			number_of_residues_cartesian, number_of_residues_dist, reference_column, ROWS, COLS;
	...
	static List<Integer> residue_list, residue_list_dist, residue_list_dist_orig, residue_list_dp1, residue_list_dp2, residue_list_dp_original1, residue_list_dp_original2, residues_read;
	...
	private static void read_batch_parameters()
		{
			...
			if (count < 2)
				{
					...
					System.err.println("Input file line 4: " + line);
					System.err.println("Terminating program execution.");
					System.exit(0);
				}

			String read = sToken.nextToken();
			if (read.equals("1") || read.equals("read")) read_PDBs = true;
			System.out.println("\tREAD PDBs = " + read_PDBs);

			String multi_chain_analysis = sToken.nextToken();
			if (multi_chain_analysis.equals("1") || multi_chain_analysis.equals("multi")) do_multi = true;
			...
			if (count < 4)
				{
					System.err.println("Expected AT LEAST 4 parameters: #Cartesian Modes, #Distance Modes, #Distance Pairs Modes, and #Modes Viz, but only got: " + count);
					System.err.println("Terminating program execution.");
					System.exit(0);
				}
			test = sToken.nextToken();
			...
			if (do_dist)
				{
					...
					if (!check)
						{
							System.err.println("The entered Distance Residue List File does not exist: " + directory + rl_dist);
							System.err.println("Terminating program execution.");
							System.exit(0);
						}
					line_count++;
			...
			for (job_num = 0; job_num < num_of_jobs; job_num++)
				{
					...
					if (do_no_pca)
						{
							System.out.println("Performing NO PCA analysis: Pre-Processing PDB files for JED Analysis... ");
							startTime = System.nanoTime();
							do_NO_PCA();
							endTime = System.nanoTime();
							totalTime = endTime - startTime;
