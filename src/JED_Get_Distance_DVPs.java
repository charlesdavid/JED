...
/**
 * JED class JED_Get_Distance_DVPs: Constructs the DVPs for the All-to-All Distance subset.
...
public class JED_Get_Distance_DVPs
{

	String directory, out_dir, description, type, file_name_head, path, Q = "COV", R = "CORR";
	int number_of_modes, reference_column, number_of_conformations, number_of_residues, ROWS, COLS;
	...
	Projector P;
	public List<Double> top_eigenvals;
	public boolean success, exist;

	/* ***************************************** CONSTRUCTOR ********************************************************************* */

	/**
	 * Constructor to get the ALL to ALL Distance delta vector projections
	 * 
	 * @param data
	 *            The matrix of subset distances
	 * @param evects
	 *            The top eigenvectors
	 * @param top_eigenvalues
	 *            The top eigenvalues
	 * @param ref
	 *            The reference column in the matrix of distances
	 * @param dir
	 *            The working directory
	 * @param des
	 *            The job description
	 * @param T
	 *            The PCA type: COV or CORR (Q or R)
	 * @param num_modes
	 *            The number of PCA modes to process
	 * @param num_res
	 *            The number of residues in the distance subset
	 */
	public JED_Get_Distance_DVPs(Matrix data, Matrix evects, List<Double> top_eigenvalues, int ref, String dir, String des, String T, int num_modes, int num_res)
		{
			...
			if (type.equals(Q))
				{
					out_dir = directory + "JED_RESULTS_" + description + "/dPCA/COV/";
			...
			if (type.equals(R))
				{
					out_dir = directory + "JED_RESULTS_" + description + "/dPCA/CORR/";
					exist = new File(out_dir).exists();
					if (!exist)
						{
							success = (new File(out_dir)).mkdirs();
	...
	/**
	 * Computes the distance delta vector series and writes it to file
	 */
	public void get_Distance_DV_Series()
		{
			...
			input_coords = null;
			path = directory + "JED_RESULTS_" + description + "/dPCA/ss_" + number_of_residues + "_delta_vectors.txt";
			Matrix_IO.write_Matrix(delta_vector_series, path, 9, 3);
		}

	/**
	 * Computes the delta vector projections and writes them to files
	 */
