...
public class JED_Get_Distance_Pair_DVPs
{
	...
	/**
	 * Method to calculate the delta vectors using the reference frame
	 */
	...
	public void get_DVPs()
		{
			...
			path = file_name_head + "_Residue_Pairs_top_" + number_of_modes + "_DVPs_" + type + ".txt";
			Matrix_IO.write_Matrix(projections, path, 9, 3);
			projections = null;
			path = file_name_head + "_Residue_Pairs_top_" + number_of_modes + "_normed_DVPs_" + type + ".txt";
			Matrix_IO.write_Matrix(normed_projections, path, 9, 3);
			normed_projections = null;
			path = file_name_head + "_Residue_Pairs_top_" + number_of_modes + "_weighted_DVPs_" + type + ".txt";
