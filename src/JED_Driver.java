...
/**
 * JED class JED_Driver: Driver program for running JED.
...
public class JED_Driver
{
	...
	static int number_Of_Input_Lines, line_count, number_of_modes_cartesian, number_of_modes_dist, number_of_modes_dist_pairs, number_of_modes_viz, number_of_residues_cartesian,
			number_of_residues_dist, number_of_residue_pairs, reference_column, ROWS, COLS;
	static String input_path, path, name, directory, out_dir, rl_cartesian, rl_dist, rl_dist_pairs, coord_input_file, description, pdb_ref_file, line, date, Q = "COV", R = "CORR";
	static double trace_COV, trace_d_COV, trace_CORR, trace_d_CORR, cond_cQ, cond_dQ, cond_cR, cond_dR, mode_amplitude, z_cut, percent;
	static List<Integer> residue_list, residue_list_dp1, residue_list_dp2, residue_list_dist, residue_list_dist_orig, residue_list_dp_original1, residue_list_dp_original2, residues_read;
	...
	static double[] pca_mode_min_COV, pca_mode_max_COV, pca_mode_min_CORR, pca_mode_max_CORR, residue_distance_means, residue_distance_std_devs;
	static Matrix original_PDB_coordinates, subset_PDB_coordinates_cartesian, weighted_pca_modes_COV, square_pca_modes_COV, weighted_square_pca_modes_COV, transformed_PDB_coordinates_cartesian,
			distance_matrix, Q_cartesian, Q_dist, R_cartesian, R_dist, top_cartesian_evectors_COV, top_distance_evectors_COV, projections_COV, normed_projections_COV, weighted_pca_modes_CORR,
			square_pca_modes_CORR, weighted_square_pca_modes_CORR, top_cartesian_evectors_CORR, top_distance_evectors_CORR, projections_CORR, normed_projections_CORR, weighted_normed_projections_COV,
			weighted_normed_projections_CORR, pca_modes_CORR, pca_modes_COV;
	static Vector<Atom> atoms;
	static boolean read_PDBs, do_cartesian, do_dist, do_dist_pairs, do_mode_viz, do_multi, do_no_pca;
	static File log;
			...
			try
				{
					...
					if (number_Of_Input_Lines < 6)
						{
							System.err.println("INSUFFICIENT DATA IN THE INPUT FILE:");
							System.err.println("THERE MUST BE 6 LINES OF PARAMETERS FOR NO-PCA (PRE-PROCESSING).");
							System.err.println("THERE MUST BE 8 LINES OF PARAMETERS FOR 1 of 3 PCA analyses.");
							System.err.println("THERE MUST BE 9 LINES OF PARAMETERS FOR 2 of 3 PCA analyses.");
							System.err.println("THERE MUST BE 10 LINES OF PARAMETERS FOR 3 of 3 PCA analyses.");
	...
	private static void read_job_parameters()
		{
			...
			if (count < 4)
				{
					System.err.println("Expected AT LEAST 4 parameters: #Cartesian Modes, #Distance Modes, #Distance Pair Modes, and #Modes Viz, but only got: " + count);
					System.err.println("Terminating program execution.");
					System.exit(0);
				}
			test = sToken.nextToken();
			...
			test = sToken.nextToken();
			OK = Test_Numeric_Type.test_Integer(test);
			if (OK) number_of_modes_dist = Integer.parseInt(test);
			System.out.println("\t#Distance Modes = " + number_of_modes_dist);
			if (!OK)
				{
					System.err.println("The number of Distance Modes MUST be an integer.");
					System.err.println("Terminating program execution.");
					System.exit(0);
				}
			test = sToken.nextToken();
			...
			if (number_of_modes_dist > 0) do_dist = true;
			if (number_of_modes_dist_pairs > 0) do_dist_pairs = true;
			if (number_of_modes_viz > 0 & do_cartesian) do_mode_viz = true;
			if (read_PDBs & do_cartesian || read_PDBs & do_dist || read_PDBs & do_dist_pairs)
				{
					System.err.println("Reading PDB files is ONLY done for Pre-Processing runs.");
					System.err.println("Terminating program execution.");
					System.exit(0);
				}
			if (read_PDBs) do_no_pca = true;
			/* *********************************************************************************************************************************** */
			if (do_cartesian & do_dist & do_dist_pairs)
				{
					if (number_Of_Input_Lines < 10)
						{
							System.err.println("Nine lines of input required for cPCA AND dPCA AND dpPCA (3 of 3), but only got " + number_Of_Input_Lines);
							System.err.println("INSUFFICIENT DATA.");
							System.err.println("One or more of the following is missing:");
							System.err.println("The Cartesian Residue List, the Distance Residue List, the Residue Pairs List, the Matrix of Coordinates with the Reference Column.");
							System.err.println("Terminating program execution.");
							System.exit(0);
						}
				}
			if ((do_cartesian & do_dist_pairs || (do_cartesian & do_dist) || do_dist & do_dist_pairs))
				{
					if (number_Of_Input_Lines < 9)
						{
							System.err.println("Nine lines of input required for BOTH cPCA and dPCA (2 of 3), but only got " + number_Of_Input_Lines);
							System.err.println("INSUFFICIENT DATA.");
							System.err.println("One or more of the following is missing:");
							System.err.println("The Cartesian Residue List, the Distance Residue List, the Matrix of Coordinates with the Reference Column.");
							System.err.println("Terminating program execution.");
							System.exit(0);
						}
				}
			if (do_cartesian ^ do_dist ^ do_dist_pairs)
				{
					if (number_Of_Input_Lines < 8)
						{
							System.err.println("Eight lines of input required for Either cPCA OR dPCA OR dpPCA (1 of 3), but only got " + number_Of_Input_Lines);
							System.err.println("INSUFFICIENT DATA.");
							System.err.println("One or more of the following is missing:");
							System.err.println("The Cartesian Residue List, the Distance Residue List, the Matrix of Coordinates with the Reference Column.");
							System.err.println("Terminating program execution.");
			...
			if (do_dist)
				{
					...
					sToken = new StringTokenizer(line);
					rl_dist = sToken.nextToken();
					System.out.println("\tDistance Residue List = " + rl_dist);
					check = new File(directory + rl_dist).exists();
					if (!check)
						{
							System.err.println("The Distance Residue List File can not be found: " + directory + rl_dist);
							System.err.println("Terminating program execution.");
							System.exit(0);
						}
					line_count++;
	...
	private static void do_cPCA()
		{
			...
			atoms = cSS.get_atoms();
			cond_cQ = cSS.get_cond_COV();
			cond_cR = cSS.get_cond_CORR();
			...
			trace_COV = cSS.getTrace_COV();
			trace_CORR = cSS.getTrace_CORR();
	...
	private static void get_cPCA_SSA()
		{
			JED_Get_Subspace_Analysis jssa = new JED_Get_Subspace_Analysis(directory, description, "cPCA", top_cartesian_evectors_COV, top_cartesian_evectors_CORR);
				{
					jssa.get_SSA_JED();
					jssa.get_FSSA_Iterated_JED();
					System.gc();
				}
		}
			...
			try
				{
					...
					log_writer.write("Condition Number of the Covariance Matrix = " + nff.format(cond_cQ) + "\n");
					log_writer.write("\tLog of the Condition Number = " + nff.format(Math.log10(cond_cQ)) + "\n");
					log_writer.write("Trace of the Correlation Matrix = " + nff.format(trace_CORR) + "\n");
					log_writer.write("Condition Number of the Correlation Matrix = " + nff.format(cond_cR) + "\n");
					log_writer.write("\tLog of the Condition Number = " + nff.format(Math.log10(cond_cR)) + "\n");
	...
	private static void do_dPCA()
		{
			JED_Do_Dist dSS = new JED_Do_Dist(directory, description, pdb_ref_file, rl_dist, reference_column, number_of_modes_dist, original_PDB_coordinates);
			dSS.set_z_cutoff(z_cut);

			if (!do_multi) dSS.do_Dist();
			if (do_multi) dSS.do_Dist_Multi();

			number_of_residues_dist = dSS.getNumber_of_residues_dist();
			residue_distance_means = dSS.getResidue_distance_means();
			residue_distance_std_devs = dSS.getResidue_distance_std_devs();
			residue_list_dist_orig = dSS.getResidue_list_dist_orig();
			residue_list_dist = dSS.getResidue_list_dist();
			if (do_multi) chain_idents = dSS.getChain_idents();
			cond_dQ = dSS.get_cond_cov();
			cond_dR = dSS.get_cond_corr();
			top_distance_evectors_COV = dSS.getTop_distance_evectors_COV();
			trace_d_COV = dSS.getTrace_dist_COV();
			top_distance_evectors_CORR = dSS.getTop_distance_evectors_CORR();
			trace_d_CORR = dSS.getTrace_dist_CORR();

			System.gc();

			if (!do_multi) write_dPCA_Log_Single();
			if (do_multi) write_dPCA_Log_Multi();

			get_dPCA_SSA();
		}

	private static void get_dPCA_SSA()
		{
			JED_Get_Subspace_Analysis jssa = new JED_Get_Subspace_Analysis(directory, description, "dPCA", top_distance_evectors_COV, top_distance_evectors_CORR);
				{
					jssa.get_FSSA_Iterated_JED();
					jssa.get_SSA_JED();
				}
	...
	private static void write_dPCA_Log_Multi()
		{
			try
				{
					log_writer.write("\nPERFORMING dPCA, Computing Top " + number_of_modes_dist + " modes." + "\n");
					log_writer.write("\nResidue list for the All-to-All Residue Distances subset:  " + rl_dist + "\n");
					log_writer.write("Number of residues in the All-to-All Residue Distances subset: " + number_of_residues_dist + "\n");
					...
					log_writer.write("Condition Number of the Covariance Matrix = " + nff.format(cond_dQ) + "\n");
					log_writer.write("\tLog of the Condition Number = " + nff.format(Math.log10(cond_dQ)) + "\n");
					log_writer.write("Trace of the Correlation Matrix = " + nff.format(trace_d_CORR) + "\n");
					log_writer.write("Condition Number of the Correlation Matrix = " + nff.format(cond_dR) + "\n");
					log_writer.write("\tLog of the Condition Number = " + nff.format(Math.log10(cond_dR)) + "\n");
					...
					for (int i = 0; i < number_of_residues_dist; i++)
						{
							for (int j = (i + 1); j < number_of_residues_dist; j++)
								{
									log_writer.write(String.format("%-12s%-16s%-16s%-16s%n", chain_idents.get(i) + residue_list_dist_orig.get(i), chain_idents.get(j) + residue_list_dist_orig.get(j),
	...
	private static void write_dPCA_Log_Single()
		{
			try
				{
					log_writer.write("\nPERFORMING dPCA, Computing Top " + number_of_modes_dist + " modes." + "\n");
					log_writer.write("\nResidue list for All-to-All Distance subset:  " + rl_dist + "\n");
					log_writer.write("Number of residues in the All-to-All Residue Distances subset: " + number_of_residues_dist + "\n");
					if (z_cut > 0) log_writer.write("The distance values with Z-scores beyond |" + z_cut + "| were set to their mean value\n\n");
					else log_writer.write("No coordinate outliers were adjusted.\n\n");
					...
					for (int i = 0; i < number_of_residues_dist; i++)
						{
							for (j = (i + 1); j < number_of_residues_dist; j++)
								{
									log_writer.write(String.format("%-12s%-16s%-16s%-16s%n", residue_list_dist_orig.get(i), residue_list_dist_orig.get(j), nf.format(residue_distance_means[k]),
	...
	private static void do_dpPCA()
		{
			...
			number_of_residue_pairs = dpSS.get_Number_of_residue_pairs();
			...
			cond_dQ = dpSS.get_cond_cov();
			cond_dR = dpSS.get_cond_corr();
	...
	private static void get_dpPCA_SSA()
		{
			JED_Get_Subspace_Analysis jssa = new JED_Get_Subspace_Analysis(directory, description, "dpPCA", top_distance_evectors_COV, top_distance_evectors_CORR);
				{
					jssa.get_FSSA_Iterated_JED();
					jssa.get_SSA_JED();
				}
	...
	public static void main(String[] args)
		{
			...
			if (do_dist)
				{
					System.out.println("Performing dPCA analysis... ");
					startTime = System.nanoTime();
					do_dPCA();
					endTime = System.nanoTime();
					totalTime = endTime - startTime;
